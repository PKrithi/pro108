    ...
    if results.multi_hand_landmarks:
        for hand_landmark in results.multi_hand_landmarks:
            ...
            for id ,lm in enumerate(hand_landmark.landmark):
                lm_list.append(lm)
              for elem_index in tipIds:
                finger_tip_y = landmarks[elem_index].y
                finger_bottom_y = landmarks[elem_index-2].y
                thumb_tip_x = landmarks[elem_index].x
                thumb_bottom_x = landmarks[elem_index-2].x
                if elem_index!=4:
